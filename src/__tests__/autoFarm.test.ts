...
xdescribe("AutoFarm BNB/CAKE lp Tests", () => {
  test("deposit BNB/CAKE lp", async () => {
  ...
  xtest("harvest BNB/CAKE lp", async () => {
    const tx = await nub.autoFarm.harvest({poolId: "243", gasPrice, amount: "0"});
    expect(tx).toBeDefined();
  });
...
xdescribe("AutoFarm CAKE/BUSD lp Tests", () => {
  ...
  xtest("harvest CAKE/BUSD lp", async () => {
    const tx = await nub.autoFarm.harvest({poolId: "381", gasPrice, amount: "0"});
    expect(tx).toBeDefined();
  });
