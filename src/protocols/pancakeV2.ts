...
export default class PancakeV2 {
  ...
  async getLpPrice(lpAddress: string) {
    const lpToken = new this.nub.web3.eth.Contract(Abi.pancakeswap.v2.lpToken, lpAddress);
    const router = new this.nub.web3.eth.Contract(this.ROUTER02_ABI, this.ROUTER02_A);
    ...
    for (let i = 0; i < 2; ++i) {
      ...
      try {
        bnbValue = (
          await router.methods.getAmountsOut(new BigNumber(10).pow(tokenDecimals), [tokens[i], this.WBNB_A]).call()
        )[1];
      } catch (err) {
        console.log(err, 'err');
      }
      ...
      let bnbPrice = (
        await router.methods.getAmountsOut(this.nub.web3.utils.toWei('1', 'ether'), [this.WBNB_A, this.BUSD_A]).call()
      )[1];
